...
这套系统每天为 gentoo-zh overlay 构建二进制包，签名后发布到镜像机，用户通过
`binrepos.conf` 取用。它每天执行，改坏了用户当天就无法安装软件包。

## 先执行检查器，它们是规则的权威

检查器是本仓库的规范。改动后至少执行与改动相关的检查器：
...
grep 依赖。**修改文案时必须同步更新断言**，否则测试失败反映的只是文案不匹配，而非行为错误。
...
要断言行为：设置桩、执行入口、检查退出码与输出。写完后**故意破坏被测代码，确认测试
真的会红**。
...
评估过但决定不做的改动记在
[`docs/evaluated-not-done.md`](docs/evaluated-not-done.md)。
...
- `DeadUrl` 指向 `www.kernel.org`：本地网络到该域名慢于 pkgcheck 的 5 秒超时，
  `cdn.kernel.org` 正常。
- `binhost-alert@` 与 `SystemdUnitFailed` 只能处理任务执行后的失败，无法发现
  任务未执行的情况。后者由 `Packages` 的 `Last-Modified` 超时覆盖。

## 不要做的

- 不 commit、不 push、不部署、不触发构建，除非维护者明确要求
- 不在 PR 或 issue 上发评论
- 不加 AI 署名
- 部署前确认三个服务都处于 inactive：
  `systemctl is-active binhost-build binhost-build-unstable binhost-kernel`
- 手动执行 `build/` 下的脚本时不要加 `sudo`——`mirror` 是 `adminc3b9c6` 的
