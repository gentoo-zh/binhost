...
EOF
    ...
    mkdir -p "${d}/overlay/.git"
    if [[ ${fetched_age_h} != none ]]; then
        : > "${d}/overlay/.git/FETCH_HEAD"
    ...
    out=$(cd "${d}" && PATH="${d}/bin:${PATH}" OVERLAY="${d}/overlay" \
        ...
        OVERLAY_FETCH_WAIT=0 TREE="${d}/tree" \
...
echo "== cycle.sh 容得下一次无法取得 overlay"
