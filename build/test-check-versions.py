...
def run(index_lines, list_lines):
...
# 新包上线：overlay 里有构建系统、不在清单里。这一项不构造索引，直接看真实
# overlay 里有没有这类包——写死一个包名会随它被收录而失效。
EXTRA_NEWCOMER = True
...
if EXTRA_NEWCOMER:
    # 用真实清单：只给一个包时，清单外那一百多个都会算成未收录的新包。
    real = pathlib.Path(sys.argv[2]) if len(sys.argv) > 2 else None
    lst = ([l.strip() for l in real.read_text().splitlines()
            if l.strip() and not l.startswith("#")] if real and real.exists()
           else ["app-admin/chezmoi"])
    rc, out = run([], lst)
    lines = [l.strip() for l in out.splitlines() if l.strip().startswith("新包")]
    ok = "✓" if lines else "✗"
    print(f"  {ok} {'新包上线未收录':<22} {'新包':<8} "
          f"{'报出 %d 个' % len(lines) if lines else '没报出来'}")
    if not lines:
        bad += 1
