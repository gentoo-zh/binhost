...
# alert.sh 里已经修过同一个问题，而这一支是它的第三份拷贝，没跟上。Telegram 对
# 超过 4096 字的消息回 400，curl -f 于是非零，末尾的 || true 把整次发送吞掉——
# 而上面那四行 journal 在 emerge 或 rustc 失败时轻易超过 4 KB。这一层是 systemd
# 把单元杀掉时才跑的，也就是脚本自己的 trap 根本没机会触发的那种情况。
if (( ${#text} > 3500 )); then
    text="${text:0:3500}
